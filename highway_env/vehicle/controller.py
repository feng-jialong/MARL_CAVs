...
class ControlledVehicle(Vehicle):
    ...
    MAX_STEERING_ANGLE = np.pi/5  # [rad] 
    DELTA_SPEED = 5  # [m/s]
    
    is_dangerous = False
    KP_F = 1   #TODO hardcode，跟车速度控制系数
    ...
    def steering_control(self, target_lane_index: LaneIndex) -> float:
        ...
        if _id > target_lane_index[2]:
            vehicle_front,vehicle_rear = self.road.neighbour_vehicles(self,(_from,_to,_id-1))
        elif _id < target_lane_index[2]:
            vehicle_front,vehicle_rear = self.road.neighbour_vehicles(self,(_from,_to,_id+1))
        else:
            vehicle_rear = vehicle_front = None  
        
        if vehicle_front is not None:
            hd_front = vehicle_front.position[0] - self.position[0]
        else:
            hd_front = 1e4  # hardcode
            
        if vehicle_rear is not None:
            hd_rear = self.position[0] - vehicle_rear.position[0]  
        else:
            hd_rear = 1e4  # hardcode
            
        if hd_rear<10 or hd_front<10:  #TODO hardcode，转向角安全控制，车距阈值
            self.is_dangerous = True
            target_lane_index = self.target_lane_index = self.lane_index
        else:
    ...
    def speed_control(self, target_speed: float) -> float:
        ...
        """
        v_lon = self.speed * np.cos(self.heading)
        v_front_lon = v_lon
        vehicle_front,_ = self.road.neighbour_vehicles(self,self.lane_index)
        if vehicle_front is not None:
            v_front_lon = vehicle_front.speed * np.cos(vehicle_front.heading)
        
        headway = self.road._compute_headway_distance(self)
        
        if (vehicle_front is not None) and (headway < 10):  #TODO hardcode，加速度安全控制，车距阈值
            v_r = 0.8 * v_front_lon  # hardcode
        else:
            v_r = target_speed
            
        if (vehicle_front is not None) and (headway < 100):   #TODO hardcode，跟车开启的范围
            a_follow = v_front_lon - v_lon
        else:
            a_follow = 0
        
        if v_r + 1 < target_speed:
            self.is_dangerous = True 
        else:
            self.is_dangerous = False
            
        return self.KP_A * (v_r - self.speed) + \
               self.KP_F * a_follow
...
class MDPVehicle(ControlledVehicle):
    ...
    SPEED_COUNT: int = 15  # 速度挡位数量
    SPEED_MIN: float = 4  # [m/s]   # 速度下限应该更低，应对突发状况
    SPEED_MAX: float = 34  # [m/s]
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        :param action: a high-level action
               info: extra information for safety control
        """
        if action == "FASTER":
