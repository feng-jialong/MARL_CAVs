...
class KinematicObservation(ObservationType):
    ...
    FEATURES: List[str] = ['is_CAV', 'is_HDV', 'x', 'y', 'vx', 'vy']
    ...
    def __init__(self, env: 'AbstractEnv',
        ...
        :param vehicles_count: Number of observed vehicles, exclude observer itself
        ...
        self.vehicles_count = vehicles_count   # CAV可观测的车辆数，可能是车群外的CAV，或者HDV
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.features_range:
            self.features_range = {
                ...
                "y": [-12, 12],
                "vx": [-1. * MDPVehicle.SPEED_MAX, 1. * MDPVehicle.SPEED_MAX],
                "vy": [-1. * MDPVehicle.SPEED_MAX, 1. * MDPVehicle.SPEED_MAX]
            }
        for feature, f_range in self.features_range.items():
            if feature in df:
                df[feature] /= f_range[1]  # 由于normalize范围的对称性，计算简化至此
                if self.clip:
                    df[feature] = np.clip(df[feature], -1, 1)
        return df

    def observe(self,is_global=False) -> np.ndarray:
        if not self.env.road:
            return np.zeros(self.space().shape)  # 据此可视化观测范围?
        origin = self.observer_vehicle if not self.absolute else None
        
        if is_global:
            df = pd.DataFrame.from_records([v.to_dict(origin,observe_intentions=self.observe_intentions) 
                                            for v in self.env.road.vehicles])
            # ege vehice的位置不需要知道，而且影响神经网络的泛化性，故置零
            df.loc[0,'x'] = 0
            df.loc[0,'y'] = 0
            
            # Features normalize and clip
            if self.normalize:
            ...
            close_vehicles = [v for v in close_vehicles if v in self.env.controlled_vehicles if v is not self.observer_vehicle]
            
            # Add CAV
            if len(close_vehicles):
                df = df.append(pd.DataFrame.from_records([v.to_dict(origin,observe_intentions=self.observe_intentions) 
            ...
            # region 感知范围内的车辆
            for v in close_vehicles:
                if np.linalg.norm(v.position-self.observer_vehicle.position)<=100 and not isinstance(v,MDPVehicle):  #TODO hardcode 感知范围
                        df = df.append(pd.DataFrame.from_records([v.to_dict(origin, observe_intentions=self.observe_intentions)]),
                                                                  ignore_index=True)
            # endregion
            '''
            close_vehicles = self.env.road.close_vehicles_to(self.observer_vehicle,
                                                             ...
                                                             count=-1,
                                                             see_behind=self.see_behind)
            close_vehicles = [v for v in close_vehicles if not isinstance(v,MDPVehicle)]
                
            if len(close_vehicles):
                df = df.append(pd.DataFrame.from_records([v.to_dict(origin, observe_intentions=self.observe_intentions)
                                                          for v in close_vehicles])[self.features],
                               ignore_index=True)
            '''
            ...
            if self.normalize:
                df = self.normalize_obs(df)
            # Fill missing rows
            max_obs = 10  # hardcode max_obs
            if df.shape[0] < max_obs:  # 还是要补齐矩阵，只是为了当成张量处理
                rows = np.zeros((max_obs-df.shape[0], len(self.features)))
...
class MultiAgentObservation(ObservationType):
    ...
    def observe(self,is_global=False) -> tuple:
        return tuple(obs_type.observe(is_global) for obs_type in self.agents_observation_types)
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    if config["type"] == "TimeToCollision":
        ...
        return MultiAgentObservation(env, config['observation_config'])
    else:
