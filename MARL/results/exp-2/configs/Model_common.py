...
class ActorNetworkPlus(nn.Module):
    def __init__(self,obs_dim,shared_size,hidden_size,action_dim,n_obs,config=None):
        ...
        self.ego_block = nn.Sequential(nn.Linear(obs_dim,shared_size),
                                    self.activator,
                                    nn.Linear(shared_size,hidden_size),
                                    self.activator)
                                    
        self.shared_block = nn.Sequential(nn.Linear(obs_dim,shared_size),
                                          self.activator,
                                          nn.Linear(shared_size,hidden_size),
                                          self.activator)

        self.hidden_block = nn.Sequential(nn.Linear(hidden_size,hidden_size),
                                          self.activator,
                                          nn.Linear(hidden_size,action_dim),
                                          nn.LogSoftmax(dim=-1))
    
    def __call__(self,state):
        state_var = state.view(-1,self.n_obs,self.obs_dim)  # 第二个参数为观测到的总车辆数(包括自身)
        out = th.sum(self.shared_block(state_var[:,1:-1,:]),dim=1)+self.ego_block(state_var[:,0,:])
        out = self.hidden_block(out)
        return  out
    
class CriticNetworkPlus(nn.Module):
    def __init__(self,obs_dim,shared_size,hidden_size,n_obs,config=None):
