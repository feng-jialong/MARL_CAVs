...
class OnPolicyReplayMemory(object):
    ...
    def _push_one(self,state,action,reward,next_state,done,hidden_state=None,global_state=None,next_global_state=None):
        if len(self.memory) < self.capacity:
            self.memory.append(None)  # 在列表中插入一个None，表示增加长度但是暂时未赋值
        mask = 0.0 if done else self.gamma
        self.memory[self.position] = Experience(state, 
                                                action, 
                                                reward, 
                                                next_state, 
                                                mask, 
    ...
    def push(self,states,actions,rewards,next_states=None,dones=None,hidden_states=None,global_states=None,next_global_states=None):
        if isinstance(states, list):
            # for state,action,reward,next_state,done in zip(states, actions, rewards, next_states, dones):
            #     self._push_one(state,action,reward,next_state,done)
            for transition in zip(states,actions,rewards,next_states,dones,hidden_states,global_states,next_global_states):
            ...
            self._push_one(states,actions,rewards,next_states,dones,hidden_states,global_states,next_global_states)
