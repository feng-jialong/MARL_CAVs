...
class MAPPO: # 默认继承object类
    ...
    def __init__(self, env, state_dim, action_dim,
        ...
        self.env.use_centralized_critic = self.config['use_centralized_critic']
        # self.env.reset()
        ...
        if self.config['use_param_share']:
            self.actor = ActorNetworkPlus(self.config['obs_dim'],self.config['actor_shared_size'],
                                          self.actor_hidden_size,self.action_dim,self.config['max_obs'],self.config)
            if self.config['use_centralized_critic']:
                self.critic = CriticNetworkPlus(self.config['obs_dim'],self.config['critic_shared_size'],
                                                self.critic_hidden_size,len(self.env.road.vehicles),self.config)
            else:
                self.critic = CriticNetworkPlus(self.config['obs_dim'],self.config['critic_shared_size'],
                                                self.critic_hidden_size,self.config['max_obs'],self.config)    #FIXME 观测车辆数在此hardcode了
        else:
            assert self.config['use_centralized_critic'] == False, "只有参数共享的架构便于使用中心化值函数"
    ...
    def interact(self):
        ...
        self.env_state,_ = self.env.reset()  # 交互前reset环境，更新scene_type后强行reset一次
        ...
        for _ in range(self.roll_out_n_episodes):
            ...
            while(True):
                action = self.exploration_action(self.env_state[0] if self.config['use_centralized_critic'] else self.env_state, 
                                                 self.n_agents)
                ...
                if self.config['use_centralized_critic']:
                    ...
                    global_states.append(self.env_state[1])
                else:
                    states.append(self.env_state)
                    global_states.append(None)
                # action
                actions.append([index_to_one_hot(a,self.action_dim) for a in action])
                # next state
                if self.config['use_centralized_critic']:
                    next_states.append(next_state[0])
                    next_global_states.append(next_state[1])
                else:
                    next_states.append(next_state)
                    next_global_states.append(None)
                # dones
                dones.append(done)
                ...
                if done:
                    ...
                    self.logger.add_scalar('info/safety',safety,self.n_episodes+self.global_step)
                    self.logger.add_scalar('info/comfort',comfort,self.n_episodes+self.global_step)
            ...
            # 判断此trajectory是否是因碰撞而提前终止
            rewards = rewards.tolist()
            if self.episode_steps[-2] < self.env.T: 
                K = self.config['collision_replay_times']  
                self.memory.push(states*K,
                                 actions*K,
                                 rewards*K,
                                 next_states*K,
                                 dones*K,
                                 hidden_states*K,
                                 global_states*K,
                                 next_global_states*K)
            else: 
                self.memory.push(states,actions,rewards,next_states,dones,hidden_states,global_states,next_global_states)
    ...
    def train(self):
        ...
        if self.config['use_rnn']: 
            #TODO: 输出层引入RNN
            hidden_states_var = to_tensor_var(batch.global_states,self.use_cuda).view(-1,self.n_agents,None).detach()
        if self.config['use_centralized_critic']:
            global_states_var = to_tensor_var(batch.global_states,self.use_cuda).detach()
            next_global_states_var = to_tensor_var(batch.global_states,self.use_cuda).detach()
        ...
        if self.config['use_gae']:  # always use gae and vary gae_lambda to switch to TD or Monte Carlo
            deltas_var = th.empty([batch_size,self.n_agents,1],dtype=th.float,device=self.device).detach()
            advantages_var = th.empty([batch_size,self.n_agents,1],dtype=th.float,device=self.device).detach()  # 分离出计算图
            returns_var = th.empty([batch_size,self.n_agents,1],dtype=th.float,device=self.device).detach()
                ...
                for i in reversed(range(batch_size)):
                    ...
                    if self.config['use_centralized_critic']:
                        deltas_var[i,agent_id,:] = rewards_var[i,agent_id,:] \
                            + self.critic_target(next_global_states_var[[i],agent_id,:,:]) * masks_var[i] \
                                - self.critic_target(global_states_var[[i],agent_id,:,:])
                    else:
                        deltas_var[i,agent_id,:] = rewards_var[i,agent_id,:] \
                            + self.critic_target(next_states_var[i,agent_id,:]) * masks_var[i] \
                                - self.critic_target(states_var[i,agent_id,:])                        
        ...
        for i in range(self.epoch_num):
            for agent_id in range(self.n_agents):
                ...
                # region tensorborad logger: actor parameters
                '''
                ...
                if self.config['critic_target_type'] == 'TD':
                    ...
                    if self.config['use_centralized_critic']:
                        next_values = self.critic_target(next_global_states_var[:,agent_id,:,:])
                    else:
                        next_values = self.critic_target(next_states_var[:,agent_id,:])
                ...
                if self.config['use_centralized_critic']:
                    values_est = self.critic(global_states_var[:,agent_id,:,:])  # critic估计value function
                else:
                    values_est = self.critic(states_var[:,agent_id,:])  # critic估计value function
                ...
                # region tensorborad logger: critic parameters
                '''
    ...
    def evaluation(self, env, output_dir, eval_episodes=1, is_train=True):
        ...
        for i in range(eval_episodes):
            ...
            while not done:
                action = self.action(state[0] if self.config['use_centralized_critic'] else state, n_agents)  # 评价时不用softmax采样
